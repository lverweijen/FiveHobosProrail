...
Mat textDetection (const Mat& input, bool dark_on_light) {
    ...
    Mat abs_grad_x( input.size(), CV_8UC1 );
    Mat abs_grad_y( input.size(), CV_8UC1 );
    Mat grad( input.size(), CV_8UC1 );

    convertScaleAbs( gradientX, abs_grad_x, 255);
    convertScaleAbs( gradientY, abs_grad_y, 255);
